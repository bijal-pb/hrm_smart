...
class LeaveApplicationsController extends AdminBaseController
{
    ...
    public function ajaxApplications()
    {
        ...
        ->editColumn('start_date', function ($row) {
            ...
            $dates = $start->format("Y-m-d") . ' ' . (isset($row->end_date) ? ' to ' . $end->format("Y-m-d") : '');

            return $dates;

        })->editColumn('applied_on', function ($row) {
            return date('Y-m-d', strtotime($row->applied_on));
        ...
        ->editColumn('leaveType', function ($row) {
            ...
            if ($row->application_status == 'pending') {
                $string = '
                <div class="d-flex">
                <button class="btn btn-outline-info btn-sm margin-bottom-5 mx-1" data-toggle="modal" href="#static_approve" onclick="show_approve(' . $row->id . ');return false;"><i class="fal fa-check"></i></button>
                <button class="btn btn-outline-warning btn-sm margin-bottom-5 mx-1" data-toggle="modal" href="#static_reject" onclick="show_reject(' . $row->id . ');return false;" > <i class="fal fa-times"></i></button>
                <button class="btn btn-outline-success btn-sm margin-bottom-5 mx-1" data-toggle="modal" href="#static" onclick="show_application(' . $row->id . ');return false;" ><i class="fal fa-edit"></i></button>
                <a  href="javascript:;" onclick="del(' . $row->id . ');return false;" class="btn btn-outline-danger btn-sm margin-bottom-5 mx-1">
                <i class="fal fa-trash"></i></a>';
            } else {
                $string = '
                        <button class="btn btn-outline-warning btn-sm waves-effect waves-themed mx-1" data-toggle="modal" href="#static" onclick="show_application(' . $row->id . ');return false;" ><i class="fal fa-eye"></i> ' . '</button>
                        <a href="javascript:;" onclick="del(' . $row->id . ');return false;" class="btn btn-outline-danger btn-sm waves-effect waves-themed mx-1">
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($leave_application->application_status == 'approved') {
            for ($i = 0; $i <= $diffDays; $i++) {
                ...
                $attendance->halfDayType = $leave_application->halfDayType == null ? 'no' : $leave_application->halfDayType;
        ...
        $this->date = ($leave_application->start_date) . ' ' . (isset($leave_application->end_date) ? ' to ' . $leave_application->end_date : '');
