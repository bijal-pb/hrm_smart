...
class ExpensesController extends AdminBaseController
{
    ...
    public function ajax_expenses()
    {
        ...
        return DataTables::of($result)
            ...
            //     $color = ['pending' => 'badge badge-danger', 'approved' => 'badge badge-success', 'rejected' => 'badge badge-danger'];
            ...
            ->addColumn('edit', function ($row) {
                ...
                $accept = '<a ' . $display_accept . ' id="accept' . $row->id . '"  data-container="body" data-placement="top" data-original-title="Approve" href="javascript:;" onclick="changeStatus(' . $row->id . ',\'approved\');return false;" class="btn btn-outline-success btn-icon waves-effect waves-themed mx-1"><i class="fal fa-check"></i></a>'; 
                $reject = '<a ' . $display_reject . ' id="reject' . $row->id . '" data-placement="top" data-original-title="Reject"  href="javascript:;" onclick="changeStatus(' . $row->id . ',\'rejected\');return false;" class="btn btn-outline-info btn-icon waves-effect waves-themed mx-1"><i class="fal fa-close"></i></a>';
                $string .= '' . $accept . $reject . '';

                $string .= '<a  class="btn btn-outline-warning btn-icon waves-effect waves-themed mx-1"  onclick="showedit(' . $row->id . ');return false;" ><i class="fal fa-edit"></i> ' . '</a>
	                    <a  href="javascript:;" onclick="del(' . $row->id . ',\'' . $row->item_name . '\');return false;" class="btn btn-outline-danger btn-icon waves-effect waves-themed mx-1">
