...
//                    ->where('employees.company_id', '=', employee()->company_id);
    ...
    // Function to find absent entry on holiday
    public static function findAbsentOnHoliday($month, $year, $employeeID, $companyId)
    {
        $holidays = Holiday::whereMonth('date',$month)->whereYear('date',$year)->where('company_id',$companyId)->pluck('date')->toArray();

        $fullday = Attendance::where('employee_id',$employeeID)->whereIn('date',$holidays)->where('status','absent')->where('halfDayType','no')->count();
        $halfday = Attendance::where('employee_id',$employeeID)->whereIn('date',$holidays)->where('status','absent')->where('halfDayType','yes')->count();
    ...
    // Function to counting the total leaves taken
