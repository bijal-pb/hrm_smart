    ...
    public function ajax_employees()
    {
        ...
        return DataTables::of($result)
            ->addColumn('edit', function ($row) {
                ...
                $string = '<div class="d-flex"><a class="btn btn-outline-warning btn-icon waves-effect waves-themed mx-1" href="' .
                    route("admin.employees.edit", $row->id) .
                    '"><i class="fa fa-edit"></i><span class="hidden-sm hidden-xs"> ' . '</span></a>
						<a class="btn btn-outline-danger btn-icon waves-effect waves-themed mx-1" href="javascript:;" onclick="del(\'' .
                    $row->id . '\',\'' . addslashes($row->full_name) .
                    '\')"><i class="fal fa-trash"></i><span class="hidden-sm hidden-xs"> ' .
                     '</span></a></div>';
...
//
    ...
    {   
        ...
        try {
            ...
            $month = date('m',strtotime($request->joining_date));
            $empId = "IML_".$month.$year.$employee->id;
            $employee->employeeID = $empId;
            $employee->save();
            if ($request->hasFile('cropImage')) {
                $file = new Files();
                $filename = $file->upload($request->file('cropImage'), 'profile_images', null, 800, false);
    ...
    public function update(UpdateRequest $request, $id)
    {
        ...
        if ($request->updateType == 'personalInfo') {
            ...
            if ($request->hasFile('cropImage')) {
                $file = new Files();
                $filename = $file->upload($request->cropImage, 'profile_images');
                $employee->profile_image = $filename;
                $employee->save();
            }
