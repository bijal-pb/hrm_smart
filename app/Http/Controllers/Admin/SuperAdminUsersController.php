...
class SuperAdminUsersController extends AdminBaseController
{
    ...
    public function ajax_superadmin_users()
    {
        ...
        return DataTables::of($result)->editColumn('created_at', function ($row) {
            ...
            if ($row->id == admin()->id) {
                $string = '<a class="btn btn-outline-warning btn-icon waves-effect waves-themed mx-1" onclick="showEdit(' . $row->id . ');return false;" >
										          <i class="fa fa-edit"></i></a>';
            } else {
                $string = '<a class="btn btn-outline-danger btn-icon waves-effect waves-themed mx-1" onclick="showEdit(' . $row->id . ');return false;" ><i class="fa fa-edit"></i> ' . trans('core.edit') . '</a>
