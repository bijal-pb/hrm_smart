...
class FaqController extends AdminBaseController
{
    ...
    public function ajaxFaqs()
    {
        ...
        return DataTables::of($result)
            ...
            $string = '<a  class="btn btn-outline-warning btn-icon waves-effect waves-themed mx-1" onclick="showEdit(' . $row->id . ');return false;" ><i class="fa fa-edit"></i></a>'.
                '<a class="btn btn-outline-danger btn-icon waves-effect waves-themed mx-1" onclick="del(\'' .
                $row->id . '\',\'' . addslashes($row->title) .
                '\')"><i class="fa fa-trash"></i><span class="hidden-sm hidden-xs"></span></a>';
            return $string;
        })
        ->rawColumns(['edit'])
        ->make();
