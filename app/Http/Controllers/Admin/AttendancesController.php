...
as a'), "a.employee_id", "=", "employees.id")
        ...
        return DataTables::of($result)
            ->addColumn('edit', function ($row) {
                ...
                $string = ' <a class="btn btn-info btn-sm" href="' . route('admin.attendances.show', $row->id) . '">
                                        <i class="fal fa-eye"></i> ' . trans("core.view") . ' </a>';
    ...
    public function ajax_attendance(Request $request)
    {
        ...
        return DataTables::of($result)
            ...
            //     return '<button type="button" class="btn btn-outline-success btn-icon waves-effect waves-themed" id="update_row' . $row->id . '" onclick="attendanceRow(\'' . $row->id . '\')"><i class="fa fa-check"></i></button>';
            // })
            ...
            ->rawColumns(['eID', 'status', 'date', 'clock_in'])
            ->make();
    }
    ...
    public function filterAttendance(Request $request)
    {
        ...
        foreach($employees as $employee) {
            ...
            $in[$employee->id.'#'.$employee->full_name] = array_fill(1, $this->daysInMonth, '-');
            $out[$employee->id.'#'.$employee->full_name] = array_fill(1, $this->daysInMonth, '-');

            foreach($employee->attendance as $attendance) {
                $final[$employee->id.'#'.$employee->full_name][Carbon::parse($attendance->date)->day]=
                    ($attendance->status == 'absent') ?
                        ($attendance->halfDayType == 'yes')? '<a href="" data-toggle="tooltip" title="Half-day Absent!"><i class="fa fa-close text-warning"></i></a>' :'<a href="#" data-toggle="tooltip" title="Full-day Absent"><i class="fa fa-close text-danger"></i>' :
                        '<i class="fa fa-check text-success"></i>';
                $in[$employee->id.'#'.$employee->full_name][Carbon::parse($attendance->date)->day] = ($attendance->clock_in == null) ? '-' : $attendance->clock_in->timezone($this->timezones[admin()->company->timezone])->format("h:i a");
                $out[$employee->id.'#'.$employee->full_name][Carbon::parse($attendance->date)->day] = ($attendance->clock_out == null) ? '-' : ($attendance->clock_out)->timezone($this->timezones[admin()->company->timezone])->format("h:i a");
            }
            $presentCount = Attendance::countPresentDays($request->month, $request->year, $employee->id);
            $holidayAbsent = Attendance::findAbsentOnHoliday($request->month, $request->year, $employee->id, $employee->company_id);
            $presentCount = $presentCount - $holidayAbsent;
            array_push($present,$presentCount);
        }
        ...
        $this->presentCounts = $present;

        $firstDay = $request->year . '-' . $request->month . '-01';
        // $presentCount = Attendance::countPresentDays($month, $year, $employeeID);
        $totalDays = date('t', strtotime($firstDay));
        $holidaycount = count(DB::select(DB::raw("SELECT * FROM holidays WHERE MONTH(date)=" . $request->month . "  AND YEAR(date)=" . $request->year . " AND company_id=" . $this->company_id)));
        $workingDays = $totalDays - $holidaycount;
