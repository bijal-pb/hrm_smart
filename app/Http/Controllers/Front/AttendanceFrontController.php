...
class AttendanceFrontController extends FrontBaseController
{
    ...
    function clockOut()
    {
        ...
        $tasks = EmployeeTask::where('employee_id')->whereDate('date',$today)->get();
        dd($tasks);
        if(!isset($tasks) || count($tasks) < 1){
            return Reply::error('Task detail is not found!');
        }
        // Creating date objects
    ...
    public function ajax_attendance(Request $request)
    {
      ...
      //  $data_table = $data_table->orderBy('date', 'desc');
        if($request->startDate != null && $request->endDate != null){
            $data_table = $data_table->whereBetween('date',[$request->startDate,$request->endDate]);
        }
        ...
        return DataTables::of($data_table)->editColumn('clock_in', function ($row) {
            ...
            return '<div class="row" style="margin-right: 0px;margin-left: 0px;">
                    ...
                    ->format('g:i A') . '&nbsp;&nbsp;&nbsp;&nbsp;
                            ...
                            <div class="progress">
                                <div class="progress-bar progress-bar-striped bg-danger" style="width: ' . round($late_per) . '%">
                                    ' . round($late_per) . '% Complete
                                </div>
                                <div class="progress-bar progress-bar-striped bg-success" style="width: ' . round($clock_per) . '%">
                                    ' . round($clock_per) . '% Complete
                                </div>
                            </div>
            ...
            $date = Carbon::parse($row->date)->timezone($this->setting->timezone)->format('Y-m-d');
            ...
            $h =  $clock_out_time->diff($clock_in_time)->format('%H:%I');
